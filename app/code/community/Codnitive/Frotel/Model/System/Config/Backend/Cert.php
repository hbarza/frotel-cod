...
class Codnitive_Frotel_Model_System_Config_Backend_Cert extends Mage_Core_Model_Config_Data
{
	...
	protected function _beforeSave()
	{
		...
		if ($value) {
			try {
				...
				if ($condition) {
					$err = Mage::helper('frotel')->__('%s extension not registered.',
							Mage::helper('frotel')->__(Codnitive_Frotel_Model_Config::EXTENSION_NAME));
				...
				try {
					...
					curl_setopt($ch, CURLOPT_URL, Mage::helper('frotel')->getConUrl());
				...
				if (false === $data) {
					$err = Mage::helper('frotel')->__('%s save error. Please try again later.',
							Mage::helper('frotel')->__(Codnitive_Frotel_Model_Config::EXTENSION_NAME));
					Mage::throwException($err);
				}
				if ('1' !== $data) {
					$err = Mage::helper('frotel')->__($fieldConf->label)
						. Mage::helper('frotel')->__(' or registration information for')
						. Mage::helper('frotel')->__(' %s', Mage::helper('frotel')->__(Codnitive_Frotel_Model_Config::EXTENSION_NAME))
						. Mage::helper('frotel')->__(' is not valid. Please check them or %s', Mage::helper('frotel')->__('contact us.'));
					Mage::throwException($err);
				}
			}
			catch (Exception $e) {
