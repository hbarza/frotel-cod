...
(function($) {
	...
	{
		frotel: {
			flipFields: function($this, type)
			{
				var frotelRegionCity = '.frotel-' + type + '-region-city';
				var mageRegionCity   = '.mage-' + type + '-region-city';

				$(mageRegionCity).removeClass('hidden');
				$(frotelRegionCity).addClass('hidden');
				if ($this.value === 'IR') {
					$(frotelRegionCity).removeClass('hidden');
					$(mageRegionCity).addClass('hidden');
				}
			},

			updateMageRegion: function(type)
			{
				var frotelSelector = '#' + type + '_frotel_region_id';
				var mageSelector   = '#' + type + ':region_id';
				if (type !== 'billing' && type !== 'shipping') {
					mageSelector = 'region_id';
				}
				var regionText     = $(frotelSelector + ' option:selected').text();
				if ($(mageSelector).hasClass('validate-select')) {
					$(mageSelector + ' option[text="' + regionText + '"]').prop('selected', true);
				}
				else {
					mageSelector = type + ':region';
					if (type !== 'billing' && type !== 'shipping') {
						mageSelector = 'region';
					}
					document.getElementById(mageSelector).value = regionText;
				}
			},

			updateMageCity: function(type)
			{
				var frotelSelector = '#' + type + '_frotel_city';
				var mageSelector   = type + ':city';
				if (type !== 'billing' && type !== 'shipping') {
					mageSelector = 'city';
				}
				var regionText     = $(frotelSelector + ' option:selected').text();
			...
			{
				alert(type);
				ldMenu(document.getElementById(type + '_frotel_region_id').selectedIndex, type + '_frotel_city');
			}
		}
	}
})(jQuery);

function loadCities(type, cityCode)
{
	ldMenu(document.getElementById(type + '_frotel_region_id').selectedIndex, type + '_frotel_city');
	(function($) {
		$('#' + type + '_frotel_city option[value="' + cityCode + '"]').prop('selected', true);
	})(jQuery);
}

function ldMenu(mySubject, cityId) {
	cityId = (cityId == undefined || cityId == 'undefined' || cityId == null) ? 'shahr' : cityId;
var Indx=mySubject;
